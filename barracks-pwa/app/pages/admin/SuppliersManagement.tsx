  ...
  return <>
    ...
    <Modal open={Boolean(profile)} title={profile?.supplier.companyName ?? "Supplier profile"} onClose={()=>setProfile(null)}>
      {profile && <div className="modal-form">
        ...
        <h3>Recent deliveries</h3>{profile.recentDeliveries.length?<div className="staff-table">{profile.recentDeliveries.map((delivery)=><div className="staff-table__row" key={delivery.id}><span><strong>Request #{delivery.id}</strong><small>{delivery.reference??"No reference"} · {delivery.branch}</small></span><span>{delivery.received_at?new Date(delivery.received_at).toLocaleString():"Received"}</span></div>)}</div>:<p>No received deliveries.</p>}
        <h3>Restock history</h3>{profile.restockHistory.length?<div className="staff-table">{profile.restockHistory.map((restock)=><div className="staff-table__row" key={restock.id}><span><strong>Request #{restock.id}</strong><small>{restock.reference??"No reference"} · {restock.branch}</small></span><span><Badge tone={restock.status==="Received"?"success":restock.status==="Cancelled"?"danger":"warning"}>{restock.status}</Badge></span><span>{restock.notes||"No notes"}</span></div>)}</div>:<p>No restock history.</p>}
      </div>}
    </Modal>
