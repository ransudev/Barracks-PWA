...
export function InventoryReports({ onToast }:{ onToast:(message:string)=>void }) {
  ...
  return <>
    ...
    <Panel>
      ...
      {loading?<div className="staff-table__empty">Loading movements…</div>:(data?.movements?.length??0)>0?<div className="staff-table staff-table--cols-5"><div className="staff-table__head"><span>Item</span><span>Movement</span><span>Stock</span><span>Branch / supplier</span><span>Recorded by</span></div>{data?.movements?.map((movement)=><div className="staff-table__row" key={movement.id}><span><strong>{movement.item_name}</strong><small>{new Date(movement.created_at).toLocaleString()}</small></span><span>{movement.movement_type} × {movement.quantity}</span><span>{movement.previous_stock} → {movement.new_stock}</span><span>{movement.branch}<small>{movement.supplier_name??"No supplier"}</small></span><span>{movement.created_by_name}</span></div>)}</div>:<EmptyState icon="box" title="No stock movements yet" description="Audited inventory operations will appear here." />}
    </Panel>
  </>;
}
