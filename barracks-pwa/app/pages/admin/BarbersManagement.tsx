...
export function BarbersManagement({ onToast, canDelete, isAdministrator }: { onToast: (message: string) => void; canDelete: boolean; isAdministrator: boolean }) {
  ...
  async function saveBarber(event: FormEvent) {
    ...
    try {
      ...
      if (isAdministrator && commissionRate !== null && (!Number.isFinite(commissionRate) || commissionRate < 0 || commissionRate > 100)) throw new Error("Commission rate must be between 0 and 100.");
      ...
      const payload = { firstName, lastName, status: form.status, ...(isAdministrator ? { commissionRate, rating } : {}) };
  ...
  const activeBarbers = items.filter((barber) => barber.status !== "unavailable").length;
  ...
  return (
    <>
      <PageHeader title="Barber management" action={<>{isAdministrator && <Button variant="secondary" iconAfter="chevronDown" onClick={openCommissionRate}>Set commission rate</Button>}<Button icon="plus" onClick={openCreate}>Add barber</Button></>} />
      <div className={`metrics-grid ${isAdministrator ? "metrics-grid--four" : "metrics-grid--two"}`}><MetricCard label="Total barbers" value={String(items.length)} icon="scissors" accent="blue" />{isAdministrator ? <><MetricCard label="Roster revenue" value={formatCurrency(totalRevenue)} icon="wallet" accent="green" /><MetricCard label="Commission rate" value={commissionRateLabel} icon="chart" accent="amber" /><MetricCard label="Total commission" value={formatCurrency(totalCommission)} icon="spark" accent="violet" /></> : <MetricCard label="Active barbers" value={String(activeBarbers)} icon="check" accent="green" />}</div>
      ...
      <Panel className="staff-table-panel barber-crud-panel">
        ...
        <div className={`barber-table ${isAdministrator ? "" : "barber-table--staff"}`}>
          <div className="barber-table__head"><span>Name</span><span>Status</span><span>Services</span>{isAdministrator && <><span>Revenue</span><span>Commission</span><span>Rating</span></>}<span>Actions</span></div>
          {loading ? <div className="staff-table__empty" role="status">Loading barbers…</div> : loadError ? <div className="staff-table__empty" role="alert">{loadError}</div> : filtered.length ? filtered.map((barber) => <div className="barber-table__row" key={barber.id}>
            ...
            <span><Badge tone={statusTone(barber.status)}>{statusLabel(barber.status)}</Badge></span><span>{barber.servicesDone}</span>{isAdministrator && <><span className="barber-table__revenue">{formatCurrency(barber.revenue)}</span><span className="barber-table__commission">{barber.commissionRate === null ? "—" : `${barber.commissionRate}%`}</span><span className="barber-table__rating">{barber.rating === null ? "—" : <><span>{barber.rating.toFixed(1)}</span><Icon name="star" size={13} /></>}</span></>}
      ...
      <Modal open={modalOpen} title={editing ? "Edit barber profile" : "Add barber"} onClose={closeEditor}>
        <form className="modal-form" onSubmit={saveBarber}><div className="form-grid form-grid--two"><TextField label="First name" required value={form.firstName} onChange={(event) => setForm({ ...form, firstName: event.target.value })} autoComplete="given-name" /><TextField label="Last name" required value={form.lastName} onChange={(event) => setForm({ ...form, lastName: event.target.value })} /></div><SelectField label="Status" required value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as BarberForm["status"] })}><option value="available">Available</option><option value="busy">Busy</option><option value="unavailable">Unavailable</option></SelectField><div className="form-grid form-grid--two">{isAdministrator && <TextField label="Commission rate (optional)" type="number" min="0" max="100" step="0.01" value={form.commissionRate} onChange={(event) => setForm({ ...form, commissionRate: event.target.value })} placeholder="e.g. 40" />}{isAdministrator && <TextField label="Rating (optional)" type="number" min="0" max="5" step="0.1" value={form.rating} onChange={(event) => setForm({ ...form, rating: event.target.value })} placeholder="e.g. 4.8" />}</div>{formError && <p className="form-error" role="alert">{formError}</p>}<div className="modal-actions"><Button variant="secondary" type="button" disabled={submitting} onClick={closeEditor}>Cancel</Button><Button type="submit" icon="check" disabled={submitting}>{submitting ? "Saving…" : "Save changes"}</Button></div></form>
      </Modal>

      <Modal open={detailsOpen && Boolean(selected)} title={selected ? displayName(selected) : "Barber details"} onClose={() => setDetailsOpen(false)}>{selected && <div className="detail-modal"><div className="detail-modal__identity"><Avatar initials={createInitials(displayName(selected))} tone="slate" size="lg" /><div><strong>{displayName(selected)}</strong><span>Barber #{selected.id}</span></div></div><div className="detail-modal__rows"><div className="detail-modal__row"><span>Status</span><Badge tone={statusTone(selected.status)}>{statusLabel(selected.status)}</Badge></div><div className="detail-modal__row"><span>Services completed</span><strong>{selected.servicesDone}</strong></div>{isAdministrator && <><div className="detail-modal__row"><span>Commission rate</span><strong>{selected.commissionRate === null ? "Not set" : `${selected.commissionRate}%`}</strong></div><div className="detail-modal__row"><span>Revenue</span><strong>{formatCurrency(selected.revenue)}</strong></div><div className="detail-modal__row"><span>Commission earned</span><strong>{barberCommission(selected) === null ? "—" : formatCurrency(barberCommission(selected) ?? 0)}</strong></div><div className="detail-modal__row"><span>Rating</span><strong>{selected.rating === null ? "Not rated" : selected.rating.toFixed(1)}</strong></div></>}</div><div className="modal-actions"><Button variant="secondary" icon="edit" onClick={() => openEdit(selected)}>Edit profile</Button></div></div>}</Modal>
