        ...
        <div className="inventory-table">
          ...
          {loading ? <div className="staff-table__empty" role="status">Loading inventory…</div> : loadError ? <div className="staff-table__empty" role="alert">{loadError}</div> : filtered.length ? filtered.map((item) => <div className="inventory-table__row" key={item.id}>
            ...
            <span className="row-actions">
              <Button size="sm" variant="secondary" onClick={() => openMovement(item, "RECEIVE")}>Receive</Button>
              {item.category === "Products" && <Button size="sm" variant="secondary" onClick={() => openMovement(item, "CUSTOMER_PURCHASE")}>Sell</Button>}
              {item.category === "Supplies" && <Button size="sm" variant="secondary" onClick={() => openMovement(item, "STAFF_USAGE")}>Staff use</Button>}
              {item.supplierId && <Button size="sm" variant="secondary" onClick={() => openRestock(item)}>Restock</Button>}
