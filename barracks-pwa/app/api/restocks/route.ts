...
export async function GET() {
  ...
  try {
    ...
    const requestedBy = user.role === "front_desk" ? user.id : undefined;
    return Response.json({ success: true, restocks: await listRestockRequests(pool, supplierId ?? undefined, requestedBy) });
  } catch (error) {
