import { requireAdministratorUser, requireRolesUser } from "@/server/auth/require-role";
...
export async function POST(request: Request) {
  const user = await requireAdministratorUser();
  if (user instanceof Response) return user;
  let body: unknown;
  ...
  if (!parsed.success) return Response.json({ success: false, message: "Invalid restock request" }, { status: 400 });
  try {
